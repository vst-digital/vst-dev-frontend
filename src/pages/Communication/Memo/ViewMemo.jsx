...
const ViewMemberMemo = (props) => {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      notify({
        msg: "Not able to send reply. Something went wrong!!",
        type: "error",
      });
    }
  };
  ...
  useEffect(() => {
    ...
    CableApp.cable = actionCable.createConsumer(
      process.env.REACT_APP_BASE_CABLE
    );
        ...
        <Grid>
          <Card className={classes.communication_wrapper}>
            ...
            <Chip
              ...
              avatar={<Avatar>{memo.sender.first_name[0]}</Avatar>}
              variant="outlined"
              label={memo.sender.email}
            />{" "}
            To: {memo.receiver.email}
            <Divider variant="fullWidth" component="div" />
            <ReactFormGenerator
              back_action="/"
              back_name="Back"
      ...
      <Container>
        ...
        <Grid>
          <form onSubmit={handleSubmit}>
            <Container
              className={classes.communication_wrapper}
              actions={
                <>
