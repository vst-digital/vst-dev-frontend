...
import { ArrowDropDown, ArrowDropUp, Category } from "@material-ui/icons";
...
if (role === "subscription_owner") {
  navList = [...siteOwnerList, ...memberList];
} else if (role === "site_owner") {
  navList = [...adminList, ...memberList];
...
const Sidebar = ({ openSidebar }) => {
  ...
  const handleResize = () =>
    setTimeout(() => setContentHeight(computeHeight(64)), 500);
  ...
  const getSidebarNav = (navList, isSubList = false, parentIndex = "") => {
    return (
      <List
        component="ul"
        disablePadding
        className={cn({ [classes.navList]: !isSubList })}
      >
        {navList.map((page, index) => {
          ...
          if (isSubMenu) {
            ...
            navItem = (
              <ListItem key={key} component={"li"} className={classes.navItem}>
                <NavLink
                ...
                >
                  {page.icon ? (
                    ...
                    <span className={classes.navTextIcon}>
                      {getInitials(page.label)}
                    </span>
                  )}
                  <Typography
                    variant="subtitle1"
  ...
  return (
    ...
    >
      <div className={classes.toolbar}>
        ...
        <Typography
          variant={"h6"}
          className={cn(classes.toolbarTitle, {
            [classes.hideText]: !openSidebar,
          })}
        >
          VST Project Management
        </Typography>
      </div>
      <Scrollbar
        autoHide
